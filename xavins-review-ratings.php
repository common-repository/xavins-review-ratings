...
Plugin URI: http://jonathanspence.com/software/wordpress-plugins/xavins-review-ratings/
Description: Adds a shortcode tag [xrr rating=4/5] to display a rating.
Version: 1.4.0
Author: Jonathan 'Xavin' Spence
Author URI: http://jonathanspence.com/
*/
...
function xrr_defaults_array() {
	$defaults = array(
		...
		'always_show_decimal'	=> get_option('xrr_always_show_decimal') );
...
function xrr_process($args = array(), $inner_text=null) {
	...
	global $xrr_grouping_atts;
	
	if ($xrr_in_grouping != true)
		$atts = shortcode_atts(xrr_defaults_array(), $args );
	else
		$atts = shortcode_atts($xrr_grouping_atts, $args );
